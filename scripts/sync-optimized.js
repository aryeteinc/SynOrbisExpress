...
async function processBatchInParallel(properties, batchSize = 5) {
  ...
  for (let i = 0; i < properties.length; i += batchSize) {
    ...
    console.log(`\nProcesando lote ${Math.floor(i/batchSize) + 1}/${Math.ceil(properties.length/batchSize)} (${i+1}-${Math.min(i+batchSize, properties.length)}/${properties.length})`);
    ...
    console.log(`Lote ${Math.floor(i/batchSize) + 1} completado: ${successCount}/${batch.length} inmuebles procesados correctamente en ${duration}ms`);
...
async function main() {
  ...
  try {
    ...
    const { activeRefs } = await processBatchInParallel(propertiesToProcess, 5);
    
    // Mostrar tiempo total de procesamiento
    const totalDuration = performance.end('procesamiento_total');
    console.log(`\nTiempo total de procesamiento: ${totalDuration}ms (${(totalDuration/1000).toFixed(2)} segundos)`);
...
async function processImages(propertyId, propertyRef, images) {
  ...
  // Usar la versión optimizada para procesar imágenes
  return processImagesOptimized(propertyId, propertyRef, images);
}
...
async function processImagesOptimized(propertyId, propertyRef, images) {
  ...
  const MAX_CONCURRENT_DOWNLOADS = 3; // Límite de descargas simultáneas
  const imagePromises = [];
  
  // Preparar todas las imágenes para procesamiento
  for (let i = 0; i < images.length; i++) {
    ...
    // Añadir promesa para procesar esta imagen
    imagePromises.push({
      orden,
      imageUrl,
      process: async () => {
        try {
          return await processImageItem(propertyId, propertyRef, imageUrl, orden, propertyFolder, existingUrlMap);
        } catch (error) {
  ...
  console.log(`Inmueble #${propertyRef}: Procesando ${imagePromises.length} imágenes en paralelo (máx ${MAX_CONCURRENT_DOWNLOADS} simultáneas)...`);
  
  // Procesar imágenes en lotes para controlar la concurrencia
  for (let i = 0; i < imagePromises.length; i += MAX_CONCURRENT_DOWNLOADS) {
    const batch = imagePromises.slice(i, i + MAX_CONCURRENT_DOWNLOADS);
    
    // Procesar este lote en paralelo
    const batchResults = await Promise.all(batch.map(img => img.process()));
    ...
    // Mostrar progreso
    console.log(`Inmueble #${propertyRef}: Procesadas ${Math.min(i + batch.length, imagePromises.length)}/${imagePromises.length} imágenes`);
  }
  
  // Mostrar tiempo total de procesamiento de imágenes
  const duration = performance.end(`imagenes_${propertyRef}`);
  console.log(`Inmueble #${propertyRef}: Procesamiento de imágenes completado en ${duration}ms`);
  
  return processedImages;
}

// Función auxiliar para procesar una imagen individual
async function processImageItem(propertyId, propertyRef, imageUrl, orden, propertyFolder, existingUrlMap) {
  try {
    ...
    if (existingUrlMap.has(imageUrl)) {
      ...
      console.log(`Inmueble #${propertyRef}: Imagen ${orden} ya existe en la base de datos (ID: ${existingImage.id})`);
      ...
      if (existingImage.url_local && fs.existsSync(existingImage.url_local)) {
        console.log(`Inmueble #${propertyRef}: Imagen ${orden} ya existe localmente en ${existingImage.url_local}`);
        
        // Devolver la imagen existente
        return {
          id: existingImage.id,
          url: imageUrl,
