  ...
  try {
    ...
    if (existingProperty) {
      ...
      if (estado_inmueble_nombre && estado_inmueble_nombre.includes('Arrendado')) {
        ...
        // El campo estado_actual_id se ha eliminado por compatibilidad con la estructura actual
    ...
    if (existingProperty) {
      ...
      const [newId] = await db('inmuebles').insert({
        ...
        // Campo uso_id eliminado porque no existe en la tabla
        estado_inmueble_id: property.estado_inmueble_id,
        // El campo estado_actual_id se ha eliminado de la estructura
        tipo_consignacion_id: property.tipo_consignacion_id,
    ...
    const propertyId = existingProperty ? existingProperty.id : (newProperty ? newProperty.id : null);
        ...
        try {
          ...
          if (existingImage) {
            await db('imagenes')
              ...
              .update({
                url_local: localPath,
                // El campo hash no existe en la tabla imagenes
                updated_at: db.fn.now()
              });
